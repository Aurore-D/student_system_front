  ...
  routes: [{
    ...
    },
    {
      ...
      children: [{
        path: '/Home/Page',
        name: 'HomePage',
        component: () => import('@/views/staticMenu/HomePage.vue'),
        meta: {
          isRouter: true,
        }
      },
    ...
    if (!router.options.isAddDynamicMenuRoutes) {
      ...
      http.menu.getMenus().then((response => {
        ...
        if (response && response.status === 200) {
          ...
          // 获取动态菜单数据
          let results = fnAddDynamicMenuRoutes(response.data)
          ...
          if (results && results.length > 0) {
            ...
            results.map(value => {
              ...
              if (!value.path) {
                value.path = `/D-${value.meta.menuId}`
                value.name = `/D-${value.meta.menuId}`
