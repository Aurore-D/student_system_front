<template>
  <div>
    ...
    <el-aside class="header-logo" :width="asideWidth">
      <div @click="$router.push({ name: 'Home' })"><!--点击跳转首页-->
        <a v-if="foldAside">
          <img src="../../assets/jxd.jpg" width="25px" alt="" style="margin-bottom:-5px">
          {{title.adminCenter}}
        </a>
        <a v-else>{{title.admin}}</a>
      </div>
    </el-aside>
    ...
    <el-aside class="aside" :width="asideWidth" :class='"icon-size-" + iconSize'>
      ...
      <el-scrollbar style="height: 100%; width: 100%;"> <!--滚动条-->
        <!--
                ...
                text-color 表示字体颜色。
            -->
        <el-menu :default-active="title.menuActiveName || 'HomePage'" :collapse="!foldAside" :collapseTransition="false"
                 background-color="#909399" text-color="white">

          <el-menu-item index="HomePage" @click="$router.push({ name: 'Home' })">
            <i class="el-icon-s-home"></i>
            <span slot="title">{{title.homePage}}</span>
          </el-menu-item>
          <DynamicMenu v-for="menu in title.dynamicRoutes" :key="menu.meta.menuId" :menu="menu"></DynamicMenu>
...
<script>
    ...
    import DynamicMenu from '@/views/dynamicMenu/DynamicMenu.vue'
        ...
        computed: {
            ...
            title() {
                return {
                    ...
                    menuActiveName: this.menuActiveName,
