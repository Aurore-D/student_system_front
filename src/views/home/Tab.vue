<template>
  ...
  -->
    ...
    <el-tabs class="tab" v-model="mainTabsActiveName"  :closable="true" @tab-click="selectedTabHandle"
             @tab-remove="removeTabHandle">
      <!-- 循环遍历标签数组，用于生成标签列表 -->
      <el-tab-pane v-for="item in mainTabs" :key="item.name" :label="item.name_zh" :name="item.name">
        <el-card class="card" shadow="hover">
          <!-- 以 http 或者 https 开头的地址，均使用 iframe 进行展示 -->
          <iframe v-if="item.type === 'iframe'" :src="item.iframeUrl" width="100%" height="420px"   frameborder="0"
                  scrolling="yes">
          </iframe>
